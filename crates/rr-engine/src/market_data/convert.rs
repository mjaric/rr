...
/// How far ahead of wall-clock `now` a trade's exchange timestamp may sit
/// before it is rejected as implausible. A corrupt future timestamp would
/// otherwise advance the candle aggregator's window watermark and blackhole
/// a pair's candles until wall-clock caught up, so it is stopped here.
const FUTURE_TOLERANCE: TimeDelta = TimeDelta::seconds(30);
...
pub enum ConvertError {
    ...
    /// The exchange timestamp is implausibly far ahead of wall-clock `now`.
    #[error("trade timestamp {ts_exchange} is more than 30s ahead of {now}")]
    FutureTimestamp {
        /// The trade's exchange timestamp.
        ts_exchange: DateTime<Utc>,
        /// Wall-clock reference supplied by the caller.
        now: DateTime<Utc>,
    },
}
...
///
/// `now` is the caller's wall-clock reference (injected so the timestamp
/// sanity check is testable); the CLI passes [`Utc::now`].
///
/// # Errors
///
/// Returns [`ConvertError::FutureTimestamp`] if `ts_exchange` is more than 30s
/// ahead of `now`, [`ConvertError::NonFinite`] if price or amount is
/// NaN/infinite, or [`ConvertError::NonPositive`] if either is zero or negative.
pub fn to_trade_record(
    event: &MarketEvent<MarketDataInstrument, PublicTrade>,
    now: DateTime<Utc>,
) -> Result<TradeRecord, ConvertError> {
    if event.time_exchange > now + FUTURE_TOLERANCE {
        return Err(ConvertError::FutureTimestamp {
...
mod tests {
    ...
    /// A wall-clock reference comfortably after the fixed event timestamp
    /// (`10:00:00.123Z`), so the future-timestamp guard accepts it.
    fn now() -> DateTime<Utc> {
    ...
    fn normal_trade_converts_every_field() -> Result<(), ConvertError> {
        let record = to_trade_record(&event(104_500.25, 0.015), now())?;
    ...
    fn coinbase_exchange_id_converts_to_snake_case_str() -> Result<(), ConvertError> {
        ...
        let record = to_trade_record(&ev, now())?;
    ...
    fn coinbase_minimum_amount_converts_exactly() -> Result<(), ConvertError> {
        ...
        let record = to_trade_record(&event(100.0, 1e-8), now())?;
    ...
    fn nan_and_infinite_price_are_non_finite_errors() {
        for bad in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            match to_trade_record(&event(bad, 1.0), now()) {
    ...
    fn zero_amount_is_a_non_positive_error() {
        match to_trade_record(&event(100.0, 0.0), now()) {
    ...
    fn negative_price_is_a_non_positive_error() {
        match to_trade_record(&event(-0.5, 1.0), now()) {
            Err(ConvertError::NonPositive { field, value }) => {
                assert_eq!(field, "price");
    ...
    fn timestamp_exactly_at_tolerance_is_accepted() -> Result<(), ConvertError> {
        // event ts is 10:00:00.123Z; a `now` exactly 30s earlier puts the
        // trade at the tolerance edge, which the strict `>` check accepts.
        let record = to_trade_record(&event(100.0, 1.0), ts("2026-06-12T09:59:30.123Z"))?;
    ...
    #[test]
    #[expect(clippy::panic, reason = "test assertion on unexpected variant")]
    fn timestamp_beyond_tolerance_is_a_future_timestamp_error() {
        // One millisecond past the edge: ts now leads `now` by >30s.
        match to_trade_record(&event(100.0, 1.0), ts("2026-06-12T09:59:30.122Z")) {
            Err(ConvertError::FutureTimestamp { ts_exchange, now }) => {
                assert_eq!(ts_exchange, ts("2026-06-12T10:00:00.123Z"));
                assert_eq!(now, ts("2026-06-12T09:59:30.122Z"));
            }
